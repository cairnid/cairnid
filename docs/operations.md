...
Capture `release-assets-verification.json` only after a tagged GitHub Release exists and the assets have been downloaded from that release. First confirm repository or organization release immutability was enabled before the release was published, then run `gh attestation verify` for provenance on release assets and for CycloneDX SBOM predicates on archive files only using signer workflow `cairnid/cairnid/.github/workflows/release.yml` and source ref `refs/tags/<tag>`. Do not run the SBOM predicate check against `SHA256SUMS.txt`, `release-manifest.json`, or SBOM JSON assets. Then generate the saved receipt from local files:
...
- `release-assets-verification.json`: token-free operator receipt captured after the public GitHub Release assets are available; must be `status="ok"`, include a valid `completed_at`, a tag matching `vMAJOR.MINOR.PATCH` or `vMAJOR.MINOR.PATCH-rc.N`, a 40-character source commit, a published GitHub Release URL, `github_release_immutability_enabled_before_publish=true`, `SHA256SUMS.txt` presence and verification, `release-manifest.json` presence and checksum verification, manifest `source.validated_ci_run_url`, four expected `cairnid` and `cairnid-mcp` archives for Linux x86_64 and Windows x86_64, four matching CycloneDX `specVersion="1.5"` SBOMs, per-asset SHA-256 verification, release-manifest entries, GitHub provenance attestation verification for release assets, and GitHub SBOM predicate attestation verification for archive files only. Do not archive `gh` debug logs, request headers, cookies, tokens, raw attestation payloads, or provider secrets in this file.
