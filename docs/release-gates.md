...
- GitHub artifact provenance attestations for release assets and archive-only CycloneDX SBOM predicate attestations created with `actions/attest@v4`, GitHub Actions OIDC, and Sigstore, without long-lived signing keys.

Maintainers must review and publish the draft before the assets are public. GitHub Release immutability must be enabled before publishing the draft because final verification requires `--github-release-immutability-enabled-before-publish`. RC tags are marked as prereleases and are not marked latest. No public crates.io packages, Homebrew formulae, MSI installers, macOS notarized binaries, signed Windows binaries, container images, install scripts, or site/runtime release artifacts exist yet.
...
The default `gh attestation verify` command checks the SLSA provenance predicate for release assets. The command with `--predicate-type https://cyclonedx.org/bom` checks the SBOM attestation for archive files only. Do not run the SBOM predicate check against `SHA256SUMS.txt`, `release-manifest.json`, or SBOM JSON assets. Run the same pattern for every published target that the user installs or records in evidence.
